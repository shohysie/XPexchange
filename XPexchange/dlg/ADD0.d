...
APPEND ~%dlgname%~
	...
	IF ~~ THEN BEGIN xpselect0
		...
		IF ~InMyArea(Player1) NightmareModeOn() XPGT(Player1,7499999)~ THEN REPLY @11 GOTO xpselect1	//是<PLAYER1>需要进行交换。
		IF ~InMyArea(Player2) NightmareModeOn() XPGT(Player2,7499999)~ THEN REPLY @12 GOTO xpselect2	//是<PLAYER2>需要进行交换。
		IF ~InMyArea(Player3) NightmareModeOn() XPGT(Player3,7499999)~ THEN REPLY @13 GOTO xpselect3	//是<PLAYER3>需要进行交换。
		IF ~InMyArea(Player4) NightmareModeOn() XPGT(Player4,7499999)~ THEN REPLY @14 GOTO xpselect4	//是<PLAYER4>需要进行交换。
		IF ~InMyArea(Player5) NightmareModeOn() XPGT(Player5,7499999)~ THEN REPLY @15 GOTO xpselect5	//是<PLAYER5>需要进行交换。
		IF ~InMyArea(Player6) NightmareModeOn() XPGT(Player6,7499999)~ THEN REPLY @16 GOTO xpselect6	//是<PLAYER6>需要进行交换。
		IF ~!NightmareModeOn()
		...
		!XPGT(Player2,7499999)
		!XPGT(Player3,7499999)
		!XPGT(Player4,7499999)
		!XPGT(Player5,7499999)
		!XPGT(Player6,7499999)~ THEN REPLY @39 GOTO xpselect9	//我的队员经验都不够充足，下次吧。		
...
*/
	IF ~~ THEN BEGIN xpexchange0
		...
		IF ~NightmareModeOn()~ THEN REPLY @61 GOTO xpexchange1	//一百五十万点。
		IF ~NightmareModeOn()~ THEN REPLY @62 GOTO xpexchange2	//二百二十五万点。
		IF ~NightmareModeOn()~ THEN REPLY @63 GOTO xpexchange3	//三百万点。
		IF ~NightmareModeOn()~ THEN REPLY @64 GOTO xpexchange4	//三百七十五万点。
		IF ~NightmareModeOn()~ THEN REPLY @65 GOTO xpexchange5	//五百万点。
		IF ~NightmareModeOn()~ THEN REPLY @66 GOTO xpexchange6	//六百万点。
		IF ~NightmareModeOn()~ THEN REPLY @67 GOTO xpexchange7	//七百五十万点。
		IF ~~ THEN REPLY @40 GOTO xpselect9	//我改变主意了，还是算了。
	END
